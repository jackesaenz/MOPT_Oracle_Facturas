        ...
        public void ConsultaExisteProveedor(string vCedulaJuridica, out string[] vDescMsjError, out bool vExiste)
        {
            ...
            try
            {
                string sql = @"SELECT DECODE(
                                            (SELECT COUNT(*)
                                            FROM SINAEP_REGISTRO_PROVEEDORES		
                                            WHERE CEDULA = '" + vCedulaJuridica + "') + "
                                  +       " (SELECT COUNT(*) "
                                  +       " FROM SIRPA_DIREF01M_PROVE"
                                  +       " WHERE NUMERO = '" + vCedulaJuridica + "')"
                                  + " ,0,0,1) AS EXISTE "
                                + " FROM DUAL";
                ...
                if (dsProv != null && dsProv.Tables.Count > 0 && dsProv.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        vExiste = row["EXISTE"].ToString().Equals("0") ? false : true;
                    }
                }
            }
            catch (Exception ex)
        ...
        public void ConsultaNombreProveedor(string vNombre, out string[] vDescMsjError, out List<Proveedores> vListaProveedores)
        {
            ...
            try
            {
                string sql = @"SELECT P1.CEDULA, P1.NOMBRE
                         ...
                         + " WHERE P2.DESCRIPCION LIKE '%" + vNombre + "%'";
